...
class AIService {
    ...
    async analyzeProductProfile(productInfo) {
        logger_1.default.info('Starting AI analysis for product profile');
        try {
...
5. DEFINA ESTRATÉGIA DE POSICIONAMENTO:
- Hierarquia visual dos elementos
- Evitar sobreposições conflitantes
- Foco no que mais converte

Seja CRIATIVO com cores e efeitos. Analise o perfil do cliente e crie uma identidade visual única que maximize conversões.`
                    },
                    {
                        role: 'user',
                        content: prompt
            ...
            logger_1.default.info(`AI analysis completed: ${analysisResult.profile} profile with ${analysisResult.confidence}% confidence`);
            return analysisResult;
        }
        catch (error) {
            logger_1.default.error('Error in AI analysis:', error);
            // Fallback analysis based on simple heuristics
            return this.fallbackAnalysis(productInfo);
        }
    }
    buildAnalysisPrompt(productInfo) {
...
- Backgrounds devem ser neutros mas que complementem as imagens enviadas`;
    ...
    fallbackAnalysis(productInfo) {
        logger_1.default.warn('Using fallback analysis due to AI service error');
        let profile = 'medio';
        let confidence = 60;
        ...
        const hasLuxuryKeywords = luxuryKeywords.some(keyword => description.includes(keyword) || targetAudience.includes(keyword));
        // Palavras-chave para baixo padrão
        const popularKeywords = ['popular', 'acessível', 'primeira casa', 'entrada facilitada', 'financiamento'];
        const hasPopularKeywords = popularKeywords.some(keyword => description.includes(keyword) || targetAudience.includes(keyword));
    ...
    extractBudgetValue(budget) {
        const match = budget.match(/(\d+)/);
        return match ? parseInt(match[1], 10) : 500; // default value
    }
    generateTitleByProfile(profile) {
        const titles = {
    ...
    generateSubtitleByProfile(profile) {
        const subtitles = {
    ...
    generateDynamicEffects(profile) {
    ...
    generateDynamicLayoutStrategy(profile) {
    ...
    async generateChatResponse(message, productInfo, images) {
        logger_1.default.info('Generating chat response', {
...
Seja sempre profissional, prestativa e focada em resultados de marketing.`;
            ...
            logger_1.default.error('Error generating chat response:', error);
